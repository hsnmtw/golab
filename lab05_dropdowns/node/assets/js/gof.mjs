...
function GameOfLife(width, high, callback) {
    if (!width || width === 0)
        width = 38;
    if (!high || high === 0)
        high = 38;
    let state = xrange(0, width).map(_ => xrange(0, high).map(_ => 0));
    //1
    state[5][1] = 1;
    state[6][1] = 1;
    state[5][2] = 1;
    ...
    function mutate() {
        const generation = xrange(0, width).map(_ => xrange(0, high).map(_ => 0));
